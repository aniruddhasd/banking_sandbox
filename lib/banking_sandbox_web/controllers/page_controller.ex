defmodule BankingSandboxWeb.PageController do
  ...
  """
  def show_transaction(conn, %{"account_id" => account_id, "transaction_id" => transaction_id} = _params) do
      token = conn |> Plug.Conn.get_req_header("authorization") |> hd
      with {:ok, transaction} <- Banking.get_customer_account_transaction(token, account_id, transaction_id) do          
        conn
          |> put_status(:ok)
          |> put_view(BankingSandboxWeb.TransactionView)
