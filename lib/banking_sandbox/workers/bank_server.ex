defmodule BankingSandbox.BankServer do
  ...
  def init(_state) do
    state = %{tokens: [], accounts: 0, transactions: {0, 0}, customers: 0}
  ...
  def handle_continue(:begin_banking, state) do
    ...
    Process.send_after(
      TransactionExecutor,
      {:transaction_call, @transaction_call_standard},
      @transaction_call_initial
    )
  ...
  def handle_call(
        :get_live_stats,
        _,
        %{accounts: accounts, transactions: transactions, customers: customers} = state
      ) do
  ...
  def handle_info({:customer_call, customer_call}, %{tokens: tokens} = state) do
    ...
    customer_call =
      if length(tokens) < @customer_call_standard_limit,
        do: customer_call,
        else: @customer_call_standard
  ...
  def handle_info({:remove_token, token}, %{tokens: tokens} = state) do
    ...
    BankingSandboxWeb.Endpoint.broadcast("banking", "customer", %{
      value: -1,
