defmodule BankingSandboxWeb.PageControllerTest do
    ...
    describe "API" do
        ...
        test "Display error when illegitimate account id is used to access transactions", %{conn: conn} do
            conn = get(conn, Routes.page_path(conn, :show_transactions, "random_account_id"))
            assert json_response(conn, 401) == %{"error" => "not_found"}
        end        

        @tag run: true
        test "Fetch specific transaction of specific account associated with a token & account ID & transaction id", %{conn: conn, token: token} do
            {:ok, accounts} = Banking.get_customer_accounts(token)
            account = accounts |> Helpers.generate_random(1)
            ...
            assert json_response(conn, 200) |> Jason.encode! == render_transaction_json("transaction.json", transaction: transaction)
        end

        test "Display error when illegitimate transaction id is used to access transactions", %{conn: conn} do
            conn = get(conn, Routes.page_path(conn, :show_transaction, "random_account_id", "random_transaction_id"))
            assert json_response(conn, 401) == %{"error" => "not_found"}
        end        
